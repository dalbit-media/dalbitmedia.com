...
const stopwords = new Set([
  "관련", "공개", "근황", "논란", "사실", "정도", "이유", "오늘", "내일", "어제", "이번", "지난", "현재", "최근", "당시", "결국", "진짜", "정말", "사람", "여성", "남성", "남자", "여자", "아이", "부모", "남편", "아내", "친구", "사진", "영상", "기사", "뉴스", "속보", "단독", "공식", "최초", "최고", "시작", "종료", "발표", "상황", "문제", "이야기", "생각", "댓글", "추천", "조회", "이벤트", "후기", "정보", "유머", "전체", "올해", "내년", "한국", "국내", "해외", "세계", "서울", "기준", "가능", "확인", "모습", "때문", "관련해", "대해서", "통해", "위해", "없는", "있는", "같은", "하는", "되는", "했다", "한다", "이라고", "그리고", "하지만", "그래서", "너무", "다시", "그냥", "아직", "무슨", "어떤", "우리", "자신", "이것", "저것", "그것", "마감", "시사회", "예고", "티저", "인터뷰", "무대", "방송", "신곡", "뮤직비디오", "영화", "요즘", "초대", "모공", "특징", "listen", "official", "teaser", "fancam", "young", "the", "mv", "jpg", "gif", "live", "short", "video", "content", "clip", "music", "news", "update", "review", "story", "episode", "channel", "poster", "cover", "full", "trailer", "album", "single", "fancam", "interview", "show", "official", "stream", "festival", "performance", "event", "watch", "tiktok", "youtube", "instagram", "sns", "인스타", "유튜브", "트위터", "페이스북", "은", "는", "이", "가", "을", "를", "의", "와", "과", "도", "만", "에", "에서", "에게", "으로", "부터", "까지", "보다", "하고", "하며", "또", "또는", "왜", "어디", "무엇", "누구", "그때", "이때", "이번에", "지난해", "작년", "내년", "지금", "바로", "여기", "저기", "얼마나", "어떻게",
]);

export const koreanProperNounSeeds = seeds as SeedRecord[];

export function normalizeProperNoun(value: string) {
  const text = value.normalize("NFKC").trim().replace(/^[@#]+/, "");
  const compact = text
    .replace(/[·&+_]+/g, " ")
  ...
  return compact ? compact.toLocaleLowerCase("ko") : "";
}
...
function tokenCandidates(text: string) {
  ...
  const addCandidate = (rawDisplayName: string, strong: boolean) => {
    const displayName = stripParticle(rawDisplayName.trim());
    const normalized = normalizeProperNoun(displayName);
    if (!normalized || seen.has(normalized) || normalized.length < 2 || stopwords.has(normalized) || /^\d+$/.test(normalized) || /\.(?:jpe?g|gif|png|webp|com|net|org)$/i.test(normalized)) return;
    if (/^[가-힣]{1,2}$/.test(normalized)) return;
    if (/^(?:the|and|for|with|from|into|this|that|news|video|official|live|watch|music|album|single|episode|drama|movie|festival|show|channel|content|stream|update|review|story|clip|photo|image|poster|cover|teaser|trailer|fancam|short|full|interview)$/i.test(normalized)) return;
    seen.add(normalized);
  ...
  for (const match of text.normalize("NFKC").matchAll(/[“‘'"]([^”’'"]{2,30})[”’'"]/gu)) {
    ...
    if (/[가-힣A-Za-z]/.test(displayName)) addCandidate(displayName, true);
  }
  for (const match of text.normalize("NFKC").matchAll(/[#@]?[가-힣A-Za-z][가-힣A-Za-z0-9·&.+-]{1,29}/gu)) {
    const original = match[0];
    const displayName = original.replace(/^[#@]/, "");
    const normalized = normalizeProperNoun(displayName);
    if (!normalized || normalized.length < 2 || stopwords.has(normalized) || /^\d+$/.test(normalized) || /\.(?:jpe?g|gif|png|webp|com|net|org)$/i.test(normalized)) continue;
    ...
    const strong = /^[#@]/.test(original)
      ...
      || (/^[가-힣]+$/.test(displayName) && displayName.length >= 2);
    addCandidate(displayName, strong);
  }
  return candidates;
}
...
export function extractProperNouns(items: Array<Pick<StreamItem, "id" | "title" | "description">>) {
  ...
  for (const { full } of itemTexts.values()) {
    const seen = new Set<string>();
    for (const candidate of tokenCandidates(full)) {
